...
class GameState extends State {
  ...
  constructor(game) {
    ...
    this.complements = ["Great", "Perfect", "Brilliant", "Awesome", "Superb", "Cool", "Fantastic", "Excellent"];
    ...
    this.hero = new Hero(canvasWidth / 2, canvasHeight - 130 + yOff, 55, 55, yOff);
    ...
    this.shareBtn = null;
    this.retryBtn = null;
    this.pauseBtn = null; //? done on pause > resume or home
    this.playBtn = null;
    this.inviteBtn = null;
    this.coinBtn = null;
    this.musicBtn = null;
  ...
  generateBtns() {
    ...
    this.pauseBtn.setText("Pause", 0, 5, 10, "center", "white");

    if (sfxValue === 1) this.musicBtn.setText("SFX : On", 0, 5, 16, "center", "white");
    if (sfxValue === 2) this.musicBtn.setText("SFX : Off", 0, 5, 16, "center", "white");

    this.coinBtn = new Button(
      canvasWidth / 2,
      canvasHeight / 2 + 85,
      78,
      40,
    ...
    this.retryBtn.setText("Home", 0, 5, 18, "center", "white");
    ...
    this.backBtn.setText("Home", 0, 5, 18, "center", "white");
  }
  ...
  generateThreeBullets(num) {
    let ang1 = -Math.PI / 2 - 0.2 + this.hero.ang,
      ang2 = -Math.PI / 2 + this.hero.ang,
      ang3 = -Math.PI / 2 + 0.2 + this.hero.ang,
      showSPBullets = true;
  ...
  handleInputs(input) {
    ...
    if (!this.gameOver && !this.continue_ && !gamePaused) {
      if (this.screenPressed) {
        ...
        if (!this.gameStart && !this.musicBtn.isClicked() && !this.sideScroller.isInUse) {
          this.startGame();
        } else {
          this.shootSound();
  ...
  update() {
    ...
    if (this.circles.length <= 0 && this.gameStart && !this.gameOver && !this.continue_) {
      this.circlePower++;
      this.circleFall += this.circleFall < 3.5 ? 0.07 : 0;
    ...
    if (this.gameOver || this.continue_) {
      if (this.endCount < 3) this.endCount += 0.2;
  ...
  render(ctx1, ctx2, ctx3, ctx4) {
    ...
    if (this.scoreQuake > 0) this.scoreQuake -= 0.25;
    ...
    for (var i = 0; i < this.coins.length; i++) {
      ...
      if (dist <= rad) {
        if (FPS >= 45) {
          let _a = this.aniNumPool.pop();
          _a.set(this.coins[i].x, this.coins[i].y - 20, "+1", 0.01, null, 0, -0.25);
          this.aninums.push(_a);
        }
        if (this.particle.length < 15) this.createSplash(4, this.coins[i].x, this.coins[i].y, 6, 12, "gold", 100, 0.5);
        this.starCount += 1;
    ...
    for (var i = 0, len = this.circles.length; i < len; i++) {
        ...
        for (var j = 0, len2 = this.circles.length; j < len2; j++) {
          if (i !== j) {
            ...
            if (distance(c.x, c1.x, c.y, c1.y) < rad && !gamePaused) {
              ...
              if (c.y <= c1.y) {
                c.yVel -= !c.isBoss ? 0.025 : 0;
                c1.yVel += !c1.isBoss ? 0.025 : 0;
              } else {
                c.yVel += !c.isBoss ? 0.025 : 0;
                c1.yVel -= !c1.isBoss ? 0.025 : 0;
              }
        ...
        for (var j = 0; j < this.pups.length; j++) {
          ...
          if (distance(c.x, p.x, c.y, p.y) < rad && !gamePaused) {
            ...
            if (c.y <= p.y) {
              ...
              c.yVel += !c.isBoss ? 0.05 : 0;
            }
          }
        }

        if (this.circles[i].y >= this.hero.y + this.circles[i].rad) {
          if (this.score >= 1000 && this.canContinue && !underChallange && !newChallange) {
            this.continue_ = true;
          } else {
            this.gameOver = true;
          ...
          if (this.particlePool.length > 0 && FPS >= 45) {
            let __p = this.particlePool.pop();
            __p.set(this.circles[i].x, this.circles[i].y, 0, 0, 30, this.circles[i].fill, 400, true, false, -40, true, null, null, null, 0.009, 0);
            this.particle.push(__p);
          }
        ...
        if (this.circles[i]) {
          ...
          if (dist <= rad) {
            if (this.circles[i].num > this.bullets[j].num) {
              var offset = Math.random() > 0.5 ? this.circles[i].initialRad - 1 : -(this.circles[i].initialRad - 1);
              if (!this.circles[i].isBoss) {
                ...
                if (this.circles[i].y > canvasHeight / 2) this.circles[i].yVel -= 0.5;
                if (this.circles[i].y < canvasHeight / 2) this.circles[i].yVel -= 0.075;
                ...
                let cNum =
                  this.circles[i].y >= canvasHeight / 2 - 100
                    ? frames % 5 == 0
                      ? this.bullets[j].num - 1
                      : this.bullets[j].num
                    : frames % 5 == 0
                      ? this.bullets[j].num + 1
                      : frames % 2 == 0
            ...
            this.createSplash(3, this.bullets[j].x, this.bullets[j].y, 4, 10, "#ffffff", 100, 0.5);
            let _b = this.bullets.splice(j, 1);
            this.bulletPool.push(_b[0]);
            j--;
            bLen--;

            this.createSplash(4, this.circles[i].x, this.circles[i].y, 8, 14, this.circles[i].fill, 100, 0.5);
      ...
      if (this.circles[i] && this.circles[i].isBoss && this.removeBoss) {
        ...
        if (this.particlePool.length > 0 && FPS >= 45) {
          let __p = this.particlePool.pop();
          __p.set(x, y, 0, 0, 30, fill, 400, true, false, -40, true, null, null, null, 0.02, 0);
          this.particle.push(__p);
        }
        this.removeAllCircles();
        this.bossEndSound();
          ...
          if (dist <= rad) {
            if (this.pups[i].num > this.bullets[j].num) {
              ...
              if (this.pups[i].label === 1) {
                ...
                let _a = this.aniNumPool.pop();
                _a.set(canvasWidth / 2, 170, " Double Speed ! ", 0.01, 40, 0, -0.3);
                this.aninums.push(_a);
                ...
                let _a = this.aniNumPool.pop();
                _a.set(canvasWidth / 2, 170, " Reflection ! ", 0.01, 40, 0, -0.3);
                this.aninums.push(_a);
                ...
                let _a = this.aniNumPool.pop();
                _a.set(canvasWidth / 2, 170, " Tripple Bullets ! ", 0.01, 40, 0, -0.3);
                this.aninums.push(_a);
                ...
                let _a = this.aniNumPool.pop();
                _a.set(canvasWidth / 2, 170, " 2 x Score ", 0.01, 60, 0, -0.3);
                this.aninums.push(_a);
              }
    ...
    if (this.gameOver && this.endCount >= 3) {
      ...
      if (this.newBest) {
        ctx4.save();
        ctx4.globalAlpha = animate(0.4, 500, 0.6, Math.PI / 2);
  ...
  drawGameName(ctx, _scr, xOff, yOff) {
    ...
    ctx.drawText("#f4d744", 45, "center", " BALL BLASTER ", xOff, canvasHeight / 2 - 180);
    ctx.drawText("#ffffff", 45, "center", " BALL BLASTER ", canvasWidth / 2, canvasHeight / 2 - 180);

    if (frames % 100 === 0) this.scoreQuake = 5;
  }

  removePup(i) {
    this.createSplash(7, this.pups[i].x, this.pups[i].y, 10, 15, this.pups[i].fill, 150, 0.5);

    if (this.particlePool.length > 0 && FPS >= 45) {
      let __p = this.particlePool.pop();
      __p.set(this.pups[i].x, this.pups[i].y, 0, 0, 30, this.pups[i].fill, 150, true, false, -1.5, true, null, null, null, 0.035, 0);
      this.particle.push(__p);
    }
