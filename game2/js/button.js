...
class Button {
   ...
   isClicked() {
      if (xDown && yDown && xDown >= this.x - this.width / 2 && xDown <= this.x + this.width / 2 && yDown >= this.y - this.height / 2 && yDown <= this.y + this.height / 2) {
         if (this.active && !this.animation) this.s = 0.92;
         return true;
      }
   ...
   update(x, y) {
      ...
      if (this.animation) this.s = animate(0.1, 1000, 1, Math.PI / 2, 0);
